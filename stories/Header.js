...
export const createHeader = ({ is_logged_in = true, notice_num = 0, onLogin }) => {
  ...
  const logo = `<div class="header-logo">
  ...
  header_inner.insertAdjacentHTML('afterbegin', logo);

  const mypage = document.createElement('p');
  mypage.className='header-mypage';
  const mypage_a = document.createElement('a');
  mypage_a.innerText="マイページ";
  mypage_a.href="#";
  ...
  mypage.appendChild(mypage_a);
  header_inner.appendChild(mypage);
